...
void napalm::cuda::handleError(cl_int err, const std::string & msg)
{
    if (err != CL_SUCCESS)
    {
...
int32_t napalm::cuda::getCLMemFlag(MemFlag mem_flag)
{
    int32_t ret = -1;
    switch (mem_flag)
    {
...
int32_t napalm::cuda::getCLMapFlag(MapMode map_mode)
{
    int32_t ret = -1;
    switch (map_mode)
    {
...
cl_channel_type napalm::cuda::getCLDataType(DataType data_type)
{
    cl_channel_type ret;
    ...
    case napalm::DATA_TYPE_UNSIGNED_INT32:
        ...
        break;
    default:
        assert(false && "Unknown cuda Data Type");
        break;
    }
...
cl_channel_order napalm::cuda::getCLChannelOrder(ImgChannelFormat channel_format)
{
...
void napalm::cuda::getCLImageForamt(ImgFormat format, ImgRegion size, cl_image_format & cl_img_f, cl_image_desc & cl_img_d)
{
