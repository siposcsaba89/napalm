...
NAPALM_OPENCL_EXPORT napalm::PlatformAndDeviceInfo napalm::cl::getPlatformAndDeviceInfo()
{
    napalm::PlatformAndDeviceInfo ret;
    ...
    if (num_platform > 0)
    {
        ...
        handleError(err, "CL Get Platform list");
        for (size_t i = 0; i < platform_ids.size(); ++i)
        {
            std::string platform_name =
                getPlatformInfo(platform_ids, int(i), CL_PLATFORM_VENDOR).c_str() + std::string(" Version:") +
                getPlatformInfo(platform_ids, int(i), CL_PLATFORM_VERSION);
            ...
            if (num_devices > 0)
            {
                ...
                {
                    std::string dev_name = getDevInfo(devices, int(j), CL_DEVICE_NAME);
